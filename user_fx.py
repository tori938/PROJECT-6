...
def best_result_by_silhouette(model_type: str,
    ...
    if model_type == 'k-means' or model_type == 'em-algorithm':
        ...
        return(f'Clusters: {highest_cluster_by_idx[0]}, Linkage: {highest_linkage_by_idx[0].capitalize()}, Silhouette Score: {round(highest_score, 3)}')
...
def best_result_by_calinski_harabasz(model_type: str,
    ...
    if model_type == 'k-means' or model_type == 'em-algorithm':
        ...
        return(f'Clusters: {highest_cluster_by_idx[0]}, Linkage: {highest_linkage_by_idx[0].capitalize()}, Calinski-Harabasz Score: {round(highest_score, 3)}')
...
def best_result_by_davies_bouldin(model_type: str,
    ...
    if model_type == 'k-means' or model_type == 'em-algorithm':
        ...
        return(f'Clusters: {smallest_cluster_by_idx[0]}, Linkage: {smallest_linkage_by_idx[0].capitalize()}, Davies-Bouldin Score: {round(smallest_score, 3)}')
...
def unpack_results(test_result: str,
                   model_type=None):
    '''
        > unpacks the best results
    '''
    
    var1, var2 = ',', ':'
    if model_type == None:
        if test_result.__contains__(var1):
            var_idx1 = test_result.find(var1)
            prms = test_result[:var_idx1]
            
        if test_result.__contains__(var2):
            var_idx1 = test_result.find(var1)
            found_metrics = test_result[var_idx1+2:]
            var_idx2 = found_metrics.find(var2)
            score = found_metrics[var_idx2+2:]
            mtrc = found_metrics[:var_idx2]
    
        return prms, mtrc, score
        ...
        if test_result.__contains__(var2):
            var_idx1 = test_result.find(var1)
            found_metrics = test_result[var_idx1+2:]
            var_idx1_2 = found_metrics.find(var1)
            found_metrics = found_metrics[var_idx1_2+1:]
            var_idx2 = found_metrics.find(var2)
            internal_score = found_metrics[var_idx2+2:]
            internal_metric = found_metrics[:var_idx2]
                
        return prms, internal_metric, internal_score